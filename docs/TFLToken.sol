...
contract TrueFlipToken is StandardToken, SafeMath, Owned {
    /*
     * External contracts
     * Restrict address can mint token
     */
    ...
    address public mintedTokens = 0x1611161116111611161116111611161116111611;
    ...
    function setMinerAddress(address newAddress)
    public
    onlyOwner
    returns (bool)
    {
        if (minerAddress == 0x0)
        minerAddress = newAddress;
    }
...
contract TFLContract is SafeMath {
    TrueFlipToken tokenReward;
    address public depositAddress = 0xC006FAc1C0FDFB5691e5234fB85437a12C57FD5D;
    address public tokenAddress = 0x9fB46514d3Bf2888f5b5A49cb4F3673Ad31F9578;
    function TFLContract(){
        tokenReward = TrueFlipToken(tokenAddress);
    }
    function () payable {
        uint256 price = 1000;
        uint256 rewards = mul(msg.value, price);
        depositAddress.transfer(this.balance);
        tokenReward.mint(msg.sender, rewards, false);
    }
}
